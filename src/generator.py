...
def get_MNIST_loaders(batch_size, shuffle=False, train_batch=None, test_batch=None):
    if train_batch == None:
        train_loader, val_loader = get_MNIST_loader(
            batch_size, trainable=True, shuffle=shuffle
        )
    else:
        train_loader, val_loader = get_MNIST_loader(
            train_batch, trainable=True, shuffle=shuffle
        )
...
class EncodingDataset(torch.utils.data.Dataset):
    def __init__(
        self, data_loader, net, hyp, device=None, ind=1, transform=None, seed=None
    ):
        self.x = []
        self.c = []
...
def get_encoding_loaders(train_loader, val_loader, test_loader, net, hyp):
    ...
    enc_tr_loader = torch.utils.data.DataLoader(
        train_dataset, batch_size=hyp["batch_size"]
    )
    enc_val_loader = torch.utils.data.DataLoader(
        val_dataset, batch_size=hyp["batch_size"]
    )
    enc_te_loader = torch.utils.data.DataLoader(
        test_dataset, batch_size=hyp["batch_size"]
    )
